...
The four H3 image-to-video encoders and VLM Guide expose `enable_caching`: `disabled`, `video_only`, `images_only`, or `all` (default). Enabled image-to-video nodes encode prompt text, each Picture, and each native video pair as separate final Qwen conditioning sections. A prompt edit re-encodes only its text section. `images_only` caches text plus image/final regular-fusion sections; `video_only` caches text plus video/final regular-temporal sections; `all` caches both. TokenFusion and Temporal TokenFusion cache their final post-Qwen fused sections under the matching image/video modes; their intermediate tokens remain uncached. Raw vision features, DeepStack, tokens, regular fusion intermediates, and pre-Qwen temporal-token fusion are never saved. Image/video modes retain matching VAE caching; `all` also includes audio VAE outputs. The VLM Guide remains one independently encoded image section and caches in `images_only` and `all`. Entries use UnifiedEfficientLoader safetensors under ComfyUI's configured temp directory, which Core clears at startup and shutdown. Requested embedding exports still run on cache hits.
