...
**MiniMax H3 Clip Continuation** saves decoded RGB tail frames and optional matching audio from clip 1 into slot `1`. Load slot `0` for clip 1 (no context), then Load slot `1` when generating clip 2. Keep one identical `filename_prefix` on both nodes. For each later clip, Load the prior slot and Save the current slot: `1→2`, `2→3`, and so on. Connect Load `continuation_media` to Clip Continuation Encoder. A video VAE and, when saved audio exists, an audio VAE are required. Tail frame 0 is a native guide and the appended Qwen Picture explicitly anchored at 00.00s. Interior tail frames are Qwen-only Video context; the final tail frame remains a native guide. Saved audio is native-only at target frame 0. Do not wire Save `path` to Load.

For an unchanged reference-video timeline, select **temporal fusion** on Clip Continuation Load. The encoder fuses saved interior frames with reference-video Qwen blocks at the same target times using the existing temporal consensus path, rather than replacing or prepending the reference. A 22-frame tail supplies 20 interior-frame alternatives; its two endpoint guides and first-frame Picture remain separate. Prompt text, reference timestamps, and requested output length are preserved. This requires an additional Qwen encoding lane and also works alongside picture fusion. `replace` and `prepend` remain available with their existing Qwen semantics and default; ordinary native reference-video guides are retained in all modes. No mode is selected automatically.

Clip Continuation Accumulate searches both the preceding output's ending and the new clip's beginning, bounded independently by `maximum_overlap_frames`. Saved `continuation_media` pixels provide supporting temporal evidence; they need not equal decoded pixels and do not shorten either search window. Sequences contain at least two frames and pass an aggregate visual threshold, so an imperfect frame need not split a match. Usable audio contributes to ranking; silent, short, incomplete, or unavailable audio is neutral. The splice minimizes visual seam discrepancy, then audio discrepancy, with central placement only as a tie-breaker. Both video and audio are trimmed at the selected positions.

The console reports cuts and explains unchanged joins. Weak saved context is reported and actual-clip evidence remains usable. Unresolved competing offsets leave both clips unchanged with a warning. Visual scoring uses mean dHash similarity and a 70/30 RGB-motion score; usable audio uses the existing 60/40 visual-audio weighting. Alignment assumes 24 fps without time stretching or dropped-frame repair. Existing matching helpers remain available. Larger search windows increase runtime; synthetic tests do not establish generated-video quality.

H3 Reference Video Components supports indexed extraction: set **Divide into segments** to `N`, then select **Segment index** from `0` through `N-1`. This divides the whole source on a shared 24-fps timeline and overrides manual duration/start controls. Segment boundaries do not drift through repeated H3 rounding. Required output padding holds the segment's last frame and adds silence; the preview reports padding separately. Set the count to `0` for existing manual behavior.

#### MiniMax H3 Ref

**MiniMax H3 Ref Extract** fuses an `IMAGE` batch into one image reference. Each image is VAE-encoded separately, then the latents are averaged into one frame before optional pooling/refinement. The default pooled mode reduces the fused VAE grid; encode keeps one full-resolution fused frame. Connect a MiniMax H3 Qwen3-VL CLIP to independently encode each image and average matching sections into one fixed-length Qwen section. This saves one ref and applies one native block plus one Qwen section regardless of batch count; averaging can lose details unique to individual views. Select `video` only when the ordered batch is one 24 fps clip; the clip needs at least five frames. Qwen Picture/Video number defaults to 17; choose distinct numbers for refs extracted in separate runs. Video keeps relative frame times and samples at 2 fps. **MiniMax H3 Audio Ref Extract** creates an independent audio reference with a matching H3 audio VAE. Compression reduces native reference token cost but loses detail; refined compression optimizes the compressed latent and does not train a model.

Use **MiniMax H3 Ref Save** under `ComfyUI/models/minimax_h3_refs`. Its `separate` layout writes one `.safetensors` per connected ref; `bundle` writes all connected image, video, and audio refs into one file. **MiniMax H3 Ref Load** returns every member of a bundle, preserving order, and also accepts original RefMod version-5 bundles. Bundling changes file placement, not the number of ref blocks at sampling. **MiniMax H3 Ref Apply** appends saved or newly extracted native refs to existing MiniMax H3 conditioning. When a ref includes stored Qwen conditioning, Apply also inserts its embeddings before the prompt, in connected ref order. The base conditioning must carry the H3 VLM layout metadata produced by this package's H3 encoder. Ref info shows each saved Picture/Video number for prompt text. Apply rejects duplicate numbers of the same media type among connected refs. `minimax_refs` counts VAE blocks, not all media already encoded by Qwen, so choose numbers above any labels used in base conditioning. Stored embeddings reflect the CLIP model and patches used at extraction; independent encoding is experimental and does not reproduce one joint Ref2VA Qwen pass. No model patcher or sampler wrapper is needed, and refs do not add learned trigger words or voice-cloning guarantees.

Ref Save is an output node, so extraction can run without a sampler. Save uses the original `refmod_meta` file header. Load accepts original-format references and earlier files saved here with `ref_meta`; the shorter node names do not change file compatibility. Each Qwen-enabled member carries embedding and tag tensors alongside its latent. Earlier label-free Qwen refs remain usable; Qwen refs with the retired timestamp or fixed number-one presentation must be re-extracted before Apply. Saved descriptions and settings are retained as metadata, not automatically applied as controls.

Refs are applied independently in socket order. `retention` controls detail retained in newly supplied native refs; zero omits their stored Qwen conditioning too. Positive retention leaves stored Qwen embeddings unchanged. It is not attention strength or a denoising curve. `max_ref_tokens` rejects excess native reference tokens instead of silently resizing or dropping refs; Qwen token counts appear separately in ref info.
