...
**MiniMax H3 Ref Extract** encodes each image in an `IMAGE` batch as a separate native H3 reference. Select `video` only when the ordered batch is one 24 fps clip; the clip needs at least five frames. Connect a MiniMax H3 Qwen3-VL CLIP to also store independent image-guide or timestamped video conditioning in each visual ref. Image guides use the configured timestamp; videos use their own relative times and sample at 2 fps. **MiniMax H3 Audio Ref Extract** creates an independent audio reference with a matching H3 audio VAE. Compression can reduce native reference token cost but loses detail; refined compression optimizes only the compressed latent and does not train a model.

Use **MiniMax H3 Ref Save** to write individual `.safetensors` artifacts under `ComfyUI/models/minimax_h3_refs`, then select an artifact with **MiniMax H3 Ref Load**. **MiniMax H3 Ref Apply** appends saved or newly extracted native refs to existing MiniMax H3 conditioning. When a ref includes stored Qwen conditioning, Apply also inserts its embeddings before the prompt, in connected ref order. The base conditioning must carry the H3 VLM layout metadata produced by this package's H3 encoder. Stored Qwen embeddings reflect the CLIP model and patches used at extraction; independent encoding is experimental and does not reproduce one joint Ref2VA Qwen pass. No model patcher or sampler wrapper is needed, and refs do not add learned trigger words or voice-cloning guarantees.

Ref Save is an output node, so extraction can run without a sampler. Save uses the original `refmod_meta` file header. Load accepts original-format references and earlier files saved here with `ref_meta`; the shorter node names do not change file compatibility. Qwen-enabled files carry two additional tensors. Saved descriptions and settings are retained as metadata, not automatically applied as controls.
