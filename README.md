...
**MiniMax H3 Ref Extract** encodes each image in an `IMAGE` batch as a separate native H3 reference. A batch is convenient for extraction but does not reduce sampling cost: every applied image adds native reference rows and, with CLIP connected, Qwen rows. Select `video` only when the ordered batch is one 24 fps clip; the clip needs at least five frames. Connect a MiniMax H3 Qwen3-VL CLIP to also store independently encoded visual conditioning in each ref. Qwen labels start at the selected Picture/Video number, default 17; image batches increment the Picture number per image. Choose distinct starting numbers for refs extracted in separate runs. Video keeps relative frame times and samples at 2 fps. **MiniMax H3 Audio Ref Extract** creates an independent audio reference with a matching H3 audio VAE. Compression can reduce native reference token cost but loses detail; refined compression optimizes only the compressed latent and does not train a model.
