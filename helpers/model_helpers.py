...
from .video_helpers import (
    VideoCompareType, audio_overlap_similarity, compute_video_signatures,
    cross_correlate_video_signatures, rank_video_overlap_candidates,
)
import numpy as np
...
def combine_minimax_h3_clip_continuations(
    ...
    if len(audio_clips) not in {0, len(clips)}:
        raise ValueError("Connect either no audio clips or one audio clip per image clip.")
    frames = []
    expected_shape = None
    for clip in clips:
        if not torch.is_tensor(clip) or clip.ndim != 4 or clip.shape[0] < 1:
            raise ValueError("Every MiniMax H3 Clip Continuation clip must be non-empty BHWC frames.")
        shape = tuple(clip.shape[1:])
        if expected_shape is None:
            expected_shape = shape
        elif shape != expected_shape:
            raise ValueError("All MiniMax H3 Clip Continuation clips must have matching frame geometry.")
        frames.append(clip)
    merged_frames = torch.cat(frames, dim=0)
    if not audio_clips:
        return merged_frames, None
    target_rate = next((audio["sample_rate"] for audio in audio_clips if audio is not None), None)
    if target_rate is None:
        return merged_frames, None
    merged_audio = []
    for clip, audio in zip(clips, audio_clips):
        if audio is None:
            waveform = torch.zeros((1, 2, round(clip.shape[0] * target_rate / fps)))
        else:
            waveform, sample_rate = audio.get("waveform"), audio.get("sample_rate")
            if not torch.is_tensor(waveform) or waveform.ndim != 3 or waveform.shape[0] != 1 or waveform.shape[1] not in {1, 2} or not isinstance(sample_rate, int) or sample_rate <= 0:
                raise ValueError("Every MiniMax H3 Clip Continuation audio clip must be mono or stereo with a positive sample rate.")
            if sample_rate != target_rate:
                waveform = torchaudio.functional.resample(waveform, sample_rate, target_rate)
            if waveform.shape[1] == 1:
                waveform = waveform.repeat(1, 2, 1)
            wanted = round(clip.shape[0] * target_rate / fps)
            waveform = waveform[..., :wanted]
            if waveform.shape[-1] < wanted:
                waveform = F.pad(waveform, (0, wanted - waveform.shape[-1]))
        merged_audio.append(waveform)
...
def snapshot_minimax_h3_clip_continuation_media(media: dict | None) -> dict | None:
    """Detach queued continuation context from caller-owned tensors."""
    if media is None:
        return None
...
def rank_minimax_h3_continuation_media_candidates(
    frames: torch.Tensor, current: torch.Tensor, maximum_frames: int,
    near_identical_tolerance: float,
) -> list[tuple[int, float]]:
    """Correlate saved target context; convert temporal offsets into leading cuts."""
    limit = min(int(maximum_frames), current.shape[0] - 1)
    if limit < 1:
        return []
    compare_type = MINIMAX_H3_CONTINUATION_CANDIDATE_COMPARE_TYPE
    reference = compute_video_signatures(frames, compare_type)
    query = compute_video_signatures(current[:limit], compare_type)
    tail_count = frames.shape[0]
    shortest = min(tail_count, limit)
    required = min(shortest, max(2, math.ceil(shortest * 0.5)))
    candidates = []
    for offset in range(tail_count - limit, tail_count - required + 1):
        result = cross_correlate_video_signatures(
            reference, query, compare_type=compare_type,
            min_overlap_fraction=0.5, min_overlap_frames=2,
            min_offset_frames=offset, max_offset_frames=offset,
        )
        if result.overlap_frames:
            candidates.append((tail_count - result.offset_frames, result.distance))
    if not candidates:
        return []
    best_distance = min(distance for _, distance in candidates)
    ceiling = best_distance + float(near_identical_tolerance) / 100.0
    return [(overlap, distance) for overlap, distance in candidates if distance <= ceiling]


def align_minimax_h3_clip_continuation_join(
    ...
    tolerance = float(near_identical_tolerance) / 100.0
    context_offset = None
    if continuation_frames is not None:
        context_signatures = compute_video_signatures(continuation_frames, compare_type)
        anchors = [
            cross_correlate_video_signatures(context_signatures, signatures, compare_type)
            for signatures in (reference_signatures, query_signatures)
        ]
        if all(
            anchor.distance <= 1.0 - minimum
            and (
                anchor.second_best_distance is None
                or anchor.second_best_distance > anchor.distance + tolerance
            )
            for anchor in anchors
        ):
            context_offset = anchors[1].offset_frames - anchors[0].offset_frames
        else:
            logger.warning("H3 join: saved-tail alignment is weak or ambiguous; using actual-clip evidence.")
    ...
    for offset in range(-query_count + 2, reference_count - 1):
        ...
        motion = 1.0 - ((first[1:] - first[:-1]) - (second[1:] - second[:-1])).abs().mean(dim=1).clamp(0, 1)
        hash_sum = [0.0, *hash_similarity.double().cumsum(0).tolist()]
        frame_sum = [0.0, *frame_similarity.double().cumsum(0).tolist()]
        motion_sum = [0.0, *motion.double().cumsum(0).tolist()]
        for start in range(count - 1):
            for stop in range(start + 2, count + 1):
                length = stop - start
                frame_score = (frame_sum[stop] - frame_sum[start]) / length
                motion_score = (motion_sum[stop - 1] - motion_sum[start]) / (length - 1)
                hash_score = (hash_sum[stop] - hash_sum[start]) / length
                score = min(hash_score, 0.7 * frame_score + 0.3 * motion_score)
                if score < minimum:
                    continue
                candidates.append((score, length, first_start + start, second_start + start, offset))
    if not candidates:
        logger.warning("H3 join unchanged: no sequence meets %.1f%% within %d/%d frames.",
                       threshold, reference_count, query_count)
        return None
    floor = max(item[0] for item in candidates) - tolerance
    by_offset = {}
    for candidate in candidates:
        if candidate[0] >= floor:
            offset = candidate[4]
            existing = by_offset.get(offset)
            if existing is None or (candidate[1], candidate[0], candidate[2]) > (existing[1], existing[0], existing[2]):
                by_offset[offset] = candidate
    if not by_offset:
        logger.warning("H3 join unchanged: no alignment satisfies the supplied tolerance %s.",
                       near_identical_tolerance)
        return None
    ranked = []
    for score, length, first_start, second_start, offset in by_offset.values():
        similarity = None
        if audio_first is not None and audio_second is not None:
            rate = audio_first[1]
            samples = round(length * rate / fps)
            left_start = round((origin + first_start) * rate / fps)
            right_start = round(second_start * rate / fps)
            left = audio_first[0][:, left_start:left_start + samples]
            right = audio_second[0][:, right_start:right_start + samples]
            if left.shape[-1] == samples and right.shape[-1] == samples:
                similarity = audio_overlap_similarity(left, right, length, fps, rate)
        if similarity is not None:
            score = 0.6 * score + 0.4 * similarity
        ranked.append((score, length, first_start, second_start, offset, similarity))
    floor = max(item[0] for item in ranked) - tolerance
    finalists = [item for item in ranked if item[0] >= floor]
    supported = [item for item in finalists if item[4] == context_offset]
    if len(supported) == 1:
        chosen = supported[0]
    elif len(finalists) == 1:
        chosen = finalists[0]
    else:
        logger.warning("H3 join unchanged: %d competing temporal alignments within %.1f%%.",
                       len(finalists), near_identical_tolerance)
        return None
    _, length, first_start, second_start, _, similarity = chosen
    seams = []
    for step in range(1, length):
        previous_keep = origin + first_start + step
        current_skip = second_start + step
        first_before = reference_features[first_start + step - 1]
        first_after = reference_features[first_start + step]
        second_before = query_features[second_start + step - 1]
        second_after = query_features[second_start + step]
        bridge = second_after - first_before
        visual_error = (
            (bridge - (first_after - first_before)).square().mean()
            + (bridge - (second_after - second_before)).square().mean()
        ).item()
        audio_error = 0.0
        if similarity is not None:
            left_cut = round(previous_keep * audio_first[1] / fps)
            right_cut = round(current_skip * audio_second[1] / fps)
            left_wave, right_wave = audio_first[0], audio_second[0]
            if 0 < left_cut < left_wave.shape[-1] and 0 < right_cut < right_wave.shape[-1]:
                bridge_audio = right_wave[:, right_cut] - left_wave[:, left_cut - 1]
                audio_error = (
                    (bridge_audio - (left_wave[:, left_cut] - left_wave[:, left_cut - 1])).square().mean()
                    + (bridge_audio - (right_wave[:, right_cut] - right_wave[:, right_cut - 1])).square().mean()
                ).item()
        seams.append((visual_error, audio_error, -min(step, length - step), step, previous_keep, current_skip))
    chosen_seam = min(seams)
    previous_keep, current_skip = chosen_seam[-2:]
    logger.warning("H3 join: remove %d previous ending frames and %d current beginning frames; matched %d; audio %s.",
                   previous.shape[0] - previous_keep, current_skip, length,
                   "scored" if similarity is not None else "neutral")
    return previous_keep, current_skip


def find_minimax_h3_clip_continuation_join(
    previous: torch.Tensor, current: torch.Tensor, threshold: float, maximum_frames: int,
    near_identical_tolerance: float = 1.0, *, continuation_frames=None,
    previous_audio=None, current_audio=None, fps: int = 24,
) -> tuple[int, int] | None:
...
def trim_minimax_h3_clip_continuation_batch(
    ...
    *, continuation_batches: Sequence[dict | None] | None = None,
    ...
    if continuation_batches is not None and len(continuation_batches) != len(image_batches):
        raise ValueError("MiniMax H3 Clip Continuation needs one continuation entry per image batch.")
    ...
    for index, (images, audio) in enumerate(zip(image_batches[1:], audio_batches[1:]), start=1):
        ...
        if join is not None:
            ...
            remove = previous.shape[0] - previous_keep
...
def find_minimax_h3_clip_continuation_overlap(
    ...
    fps: int = 24,
    near_identical_tolerance: float = 1.0,
    *, candidate_overlaps: Sequence[tuple[int, float]] | None = None,
) -> int:
    ...
    audio windows reject the candidate rather than producing a false perfect match.
    """
    ...
    if candidate_overlaps is None:
        ...
        candidate_overlaps = [
            (overlap, distance) for overlap, distance in candidate_overlaps
            if 1 <= overlap <= limit
        ]
    ...
    if audio_enabled and (
        ...
        raise ValueError("MiniMax H3 Clip Continuation overlap audio must have matching sample rates and channels.")
    valid_candidates = []
    for overlap, _candidate_distance in candidate_overlaps:
...
def _continuation_visual_features(frames: torch.Tensor) -> torch.Tensor:
    ...
    samples = F.interpolate(samples, size=(24, 24), mode="area")
...
def _load_minimax_h3_clip_continuation_media_path(path: Path) -> dict:
    with MemoryEfficientSafeOpen(str(path), low_memory=True) as handle:
        if set(handle.keys()) not in ({"frames"}, {"frames", "audio"}):
            raise ValueError("MiniMax H3 Clip Continuation file must contain frames and optional audio only.")
        ...
        with closing(handle.async_stream(names, batch_size=1, prefetch_batches=1, pin_memory=False)) as stream:
            for batch in stream:
                try:
                    if len(batch) != 1 or batch[0][0] not in {"frames", "audio"}:
                        raise ValueError("MiniMax H3 Clip Continuation stream returned an unexpected tensor.")
                    if batch[0][0] == "frames":
                        frames = batch[0][1].detach().clone()
                    else:
                        audio = batch[0][1].detach().clone()
                finally:
    ...
    result = {"format_version": 1, "frame_rate": 24, "frames": frames}
    if audio is not None:
...
def _validate_minimax_h3_ref(ref: dict) -> dict:
    ...
    if embedding is not None or tags is not None:
        ...
        presentation = validated["metadata"].get("vlm_presentation")
        if presentation in {"image_numbered", "video_numbered_2fps"}:
            number = validated["metadata"].get("vlm_reference_number")
            if (presentation == "image_numbered") != (kind == "image") or isinstance(number, bool) or not isinstance(number, numbers.Integral) or number < 17:
                raise ValueError("MiniMax H3 Ref Qwen number or media type is invalid.")
...
def create_minimax_h3_video_ref(video: torch.Tensor, vae, compression: str = "encode", grid_long_edge: int = 16, latent_frames: int = 16, refine_steps: int = 100, description: str = "", clip=None, vlm_resolution: int = 384, vlm_reference_start: int = 17) -> dict:
    ...
    if clip is not None:
        ref["vlm_embedding"], ref["vlm_tags"] = encode_minimax_h3_ref_vlm(clip, "video", _frames, vlm_resolution, vlm_reference_start)
        ref["metadata"].update({"vlm_presentation": "video_numbered_2fps", "vlm_resolution": vlm_resolution, "vlm_reference_number": vlm_reference_start})
...
def save_minimax_h3_ref_collection(refs: list[dict], filename_prefix: str, bundle: bool = False) -> list[str]:
    ...
    if bundle:
        if len(refs) > 256:
            raise ValueError("MiniMax H3 Ref bundle cannot contain more than 256 references.")
        refs = [_validate_minimax_h3_ref(ref) for ref in refs]
        header = {
            "_format_version": 5, "kind": "bundle", "name": prefix.name,
            "members": [_minimax_h3_ref_storage_metadata(ref) for ref in refs],
        }
        metadata = {MINIMAX_H3_REF_METADATA_KEY: json.dumps(header, separators=(",", ":"), sort_keys=True)}
        next_index = 1
        while True:
            ...
            try:
                with IncrementalSafetensorsWriter(temporary, metadata=metadata, max_workers=1) as writer:
                    for index, ref in enumerate(refs):
                        writer.write(f"ref_{index}", ref["latent"])
                        if "vlm_embedding" in ref:
                            writer.write(f"ref_{index}.vlm_embedding", ref["vlm_embedding"])
                            writer.write(f"ref_{index}.vlm_tags", ref["vlm_tags"])
                if os.name == "nt":
...
def load_minimax_h3_ref_collection(filename: str) -> list[dict]:
    path = _minimax_h3_ref_load_path(filename)
    with MemoryEfficientSafeOpen(path, low_memory=True) as handle:
        headers = handle.metadata() or {}
        header = headers.get(MINIMAX_H3_REF_METADATA_KEY, headers.get("ref_meta"))
        ...
        if stored.get("kind") != "bundle":
            return [load_minimax_h3_ref(filename)]
        members = stored.get("members")
        if stored.get("_format_version") != 5 or not isinstance(members, list) or not 1 <= len(members) <= 256:
            raise ValueError("MiniMax H3 Ref bundle has an unsupported member list.")
        expected = set()
        for index, member in enumerate(members):
            if not isinstance(member, dict) or member.get("kind") not in {"image", "video", "audio"}:
                raise ValueError("MiniMax H3 Ref bundle has an invalid member.")
            key = f"ref_{index}"
            expected.add(key)
            shape = handle.get_shape(key) if key in handle.keys() else ()
            if "shape" in member and member["shape"] != list(shape):
                raise ValueError("MiniMax H3 Ref bundle member shape does not match its tensor.")
            if "vlm_shape" in member:
                expected.update({f"{key}.vlm_embedding", f"{key}.vlm_tags"})
        if set(handle.keys()) != expected:
            raise ValueError("MiniMax H3 Ref bundle has an invalid tensor set.")
        for index, member in enumerate(members):
            key = f"ref_{index}"
            shape = handle.get_shape(key)
            if member["kind"] == "audio":
                valid = len(shape) == 4 and tuple(shape[:3]) == (1, 32, 2) and shape[-1] > 0
            else:
                valid = len(shape) == 5 and tuple(shape[:2]) == (1, 24) and shape[2] > 0 and min(shape[3:]) >= 2 and not shape[3] % 2 and not shape[4] % 2
                valid = valid and (member["kind"] != "image" or shape[2] == 1)
            if not valid or not handle.get_dtype(key).is_floating_point:
                raise ValueError("MiniMax H3 Ref bundle member has an invalid latent tensor.")
            if "vlm_shape" in member:
                vlm_shape = handle.get_shape(f"{key}.vlm_embedding")
                tag_shape = handle.get_shape(f"{key}.vlm_tags")
                if (member["kind"] == "audio" or len(vlm_shape) != 3 or vlm_shape[0] != 1 or min(vlm_shape) < 1
                        or tag_shape != (vlm_shape[1],) or not handle.get_dtype(f"{key}.vlm_embedding").is_floating_point
                        or handle.get_dtype(f"{key}.vlm_tags") != torch.long):
                    raise ValueError("MiniMax H3 Ref bundle member has invalid VLM tensors.")
        tensors = {}
        with closing(handle.async_stream(sorted(expected), batch_size=1, prefetch_batches=1, pin_memory=False)) as stream:
            for batch in stream:
                try:
                    if len(batch) != 1 or batch[0][0] not in expected:
                        raise ValueError("MiniMax H3 Ref bundle stream returned an unexpected tensor.")
                    tensors[batch[0][0]] = batch[0][1].detach().clone()
                finally:
                    for name, _tensor in batch:
                        handle.mark_processed(name)
        if set(tensors) != expected:
            raise ValueError("MiniMax H3 Ref bundle stream did not return all tensors.")
    refs = []
    for index, member in enumerate(members):
        key = f"ref_{index}"
        latent = tensors[key]
        if latent.ndim != (4 if member["kind"] == "audio" else 5):
            raise ValueError("MiniMax H3 Ref bundle member has an invalid latent rank.")
        dimensions = (
            {"latent_t": latent.shape[-1], "latent_h": 0, "latent_w": 0}
            if member["kind"] == "audio"
            else {"latent_t": latent.shape[2], "latent_h": latent.shape[3], "latent_w": latent.shape[4]}
        )
        if any(field in member and member[field] != value for field, value in dimensions.items()):
            raise ValueError("MiniMax H3 Ref bundle member dimensions do not match its tensor.")
        metadata = member.get("metadata", {})
        if not isinstance(metadata, dict):
            raise ValueError("MiniMax H3 Ref bundle member metadata must be a dictionary.")
        metadata = dict(metadata)
        for field in ("name", "description", "source", "source_shape", "tags", "concept_type", "sample_rate", "refmod_config"):
            if field in member:
                metadata.setdefault(field, member[field])
        steps = member.get("optimize_steps", 0)
        metadata.setdefault("refine_steps", steps)
        mode = member.get("mode", "encode")
        metadata.setdefault("compression", "encode" if mode in {"encode", "full"} else "refined" if steps else "pooled")
        ref = {"kind": member["kind"], "latent": latent, "metadata": metadata}
        if "vlm_shape" in member:
            ref["vlm_embedding"] = tensors[f"{key}.vlm_embedding"]
            ref["vlm_tags"] = tensors[f"{key}.vlm_tags"]
            if member["vlm_shape"] != list(ref["vlm_embedding"].shape) or member.get("vlm_tags") != ref["vlm_tags"].shape[0]:
                raise ValueError("MiniMax H3 Ref bundle member VLM metadata does not match its tensors.")
        refs.append(_validate_minimax_h3_ref(ref))
    return refs
...
def format_minimax_h3_ref_info(refs: list[dict]) -> str:
    ...
    for ref in refs:
        ...
        if "vlm_embedding" in ref:
            ...
            summary += f"; Qwen {label}{ref['vlm_embedding'].shape[1]} tokens"
...
def apply_minimax_h3_refs_to_conditioning(conditioning, refs: list[dict], retention: float = 1.0, max_ref_tokens: int = 0):
    ...
    if float(retention) > 0:
        ...
        for ref in validated_refs:
            if "vlm_embedding" in ref:
                presentation = ref["metadata"].get("vlm_presentation")
                if presentation not in {"image_visual", "video_visual_2fps", "image_numbered", "video_numbered_2fps"}:
                    raise ValueError("MiniMax H3 Ref Qwen presentation is outdated; re-extract the reference.")
                if presentation in {"image_numbered", "video_numbered_2fps"}:
                    label = (ref["kind"], ref["metadata"]["vlm_reference_number"])
                    if label in used_labels:
                        raise ValueError(f"MiniMax H3 Ref has duplicate {label[0]} number {label[1]}.")
