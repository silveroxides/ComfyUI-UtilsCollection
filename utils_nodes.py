...
from .model_helpers import (
    ...
    find_minimax_h3_clip_continuation_overlap,
...
class UC_MiniMaxH3ClipContinuationAccumulate(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input("overlap_threshold", display_name="Remove frames when similarity >= (%)", default=88.0, min=0.0, max=100.0, step=0.1, tooltip="When the average similarity between the previous clip's end and the current clip's start reaches this percentage, those repeated starting frames are removed. Higher values require a closer match."),
    ...
    @classmethod
    def execute(cls, images, audio=None, target_batches=2, overlap_threshold=88.0, maximum_overlap_frames=56, first_batch_reset=False, unique_id=None):
        ...
        if state["image_batches"]:
            overlap = find_minimax_h3_clip_continuation_overlap(
                torch.cat(state["image_batches"], dim=0), images, overlap_threshold, maximum_overlap_frames
            )
            if overlap:
